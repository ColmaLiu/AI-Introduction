...
class Attention(Node):
    ...
    def cal(self, X):
        ...
        K=X@W_k
        V=X@W_v
        score=Q@K.transpose(0,2,1)/np.sqrt(self.dim)
        softmax_score=np.exp(score-np.max(score,axis=-1,keepdims=True))
        softmax_score/=softmax_score.sum(axis=-1,keepdims=True)
        self.cache.append((X,softmax_score,Q,K,V))
        return softmax_score@V+X
        # TODO: YOUR CODE HERE
        # 忽略线性映射的偏置项且需要包括残差连接
...
class ResLinear(Node):
    ...
    def cal(self, X):
        ...
        # self.cache.append(X.copy())
        # X=X.reshape(B,L*D)
        # return (X@self.params[0]+self.params[1]+X).reshape(B,L,D)
        self.cache.append(X)
    ...
    def backcal(self, grad):
        ...
        # B,L,D=X.shape[0],X.shape[1],X.shape[2]
        # X=X.reshape(B,L*D)
        # self.grad[0]=X.T@grad.reshape(B,L*D)
        # self.grad[1]=np.sum(grad.reshape(B,L*D),axis=0)
        # return (grad.reshape(B,L*D)@weight.T+grad.reshape(B,L*D)).reshape(B,L,D)
        self.grad[0]=np.sum(np.expand_dims(grad,axis=-2)*np.expand_dims(X,axis=-1),axis=(0,1))
        self.grad[1]=np.sum(grad,axis=(0,1))
        return grad@weight.T+grad
        # TODO: YOUR CODE HERE
        # 提示：相比于线性层，增加了残差连接
        # 即Linear(X)+X
