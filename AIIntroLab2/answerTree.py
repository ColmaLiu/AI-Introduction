...
def entropy(Y: np.ndarray):
    ...
    """
    # TODO: YOUR CODE HERE
    lables,counts=np.unique(Y,return_counts=True)
...
def gain(X: np.ndarray, Y: np.ndarray, idx: int):
    ...
    weighted_entropy=0
    for val,prob in zip(ufeat,featp):
        subset_idx=np.where(feat==val)[0]
        subset_label=Y[subset_idx]
        subset_entropy=entropy(subset_label)
        weighted_entropy+=prob*subset_entropy
    ret = entropy(Y)-weighted_entropy
    return ret
...
def buildTree(X: np.ndarray, Y: np.ndarray, unused: List[int], depth: int, purity_bound: float, gainfunc: Callable, prefixstr=""):
    ...
    # 当达到终止条件时，返回叶节点
    if depth==0 or len(unused)==0 or np.max(ucnt)/len(Y)>=purity_bound:
        return root
    ...
    for val in ufeat:
        new_X=X[feat==val]
        new_Y=Y[feat==val]
        root.children[val]=buildTree(new_X,new_Y,unused,depth-1,purity_bound,gainfunc,prefixstr+' ')
    return root
