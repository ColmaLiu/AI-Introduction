...
class MinimaxAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        def maximizer(state, depth, index_of_agent):
            ...
            # initialize value
            value = -float('inf');"*** YOUR CODE HERE ***"
            # for every legal action, update value and maxiAction
            "*** YOUR CODE HERE ***"
            for action in state.getLegalActions(index_of_agent):
                tmp=minimizer(state.generateSuccessor(index_of_agent,action),depth,1)
                if tmp[0]>value:
                    value,maxiAction=tmp[0],action
        ...
        def minimizer(state, depth, index_of_agent):
            ...
            for action in state.getLegalActions(index_of_agent):
                ...
                if index_of_agent==state.getNumAgents()-1:
                    tmp=maximizer(state.generateSuccessor(index_of_agent,action),depth-1,0)
                else:
                    tmp=minimizer(state.generateSuccessor(index_of_agent,action),depth,index_of_agent+1)
                if tmp[0]<value:
                        value,miniAction=tmp[0],action
...
class AlphaBetaAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        def maximizer(state, depth, index_of_agent, alpha, beta):
            ...
            "*** YOUR CODE HERE ***"
            for action in state.getLegalActions(index_of_agent):
                tmp=minimizer(state.generateSuccessor(index_of_agent,action),depth,1,alpha,beta)
                if tmp[0]>value:
                    value,maxiAction=tmp[0],action
                if value>beta:
                    return (value, maxiAction)
        ...
        def minimizer(state, depth, index_of_agent, alpha, beta):
            ...
            for action in state.getLegalActions(index_of_agent):
                tmp=None
                if index_of_agent==state.getNumAgents()-1:
                    tmp=maximizer(state.generateSuccessor(index_of_agent,action),depth-1,0,alpha,beta)
                else:
                    tmp=minimizer(state.generateSuccessor(index_of_agent,action),depth,index_of_agent+1,alpha,beta)
                if tmp[0]<value:
                        value,miniAction=tmp[0],action
                if value<alpha:
                    return (value, miniAction)
        ...
        alpha = -float('inf'); "*** YOUR CODE HERE ***"
        beta = float('inf');"*** YOUR CODE HERE ***"
...
class MCTSAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState, mcts_time_limit=10):
        ...
        def Selection(cgs, cgstree):
            ...
            '''
            def isCompletelyExpanded(cgstree):
                legal_actions = cgstree.statevalue.getLegalActions(0)
                if legal_actions==[]:
                    return False
                for action in legal_actions:
                    match action:
                        case "North" if cgstree.north is None:
                            return False
                        case "East" if cgstree.east is None:
                            return False
                        case "West" if cgstree.west is None:
                            return False
                        case "South" if cgstree.south is None:
                            return False
                        case "Stop" if cgstree.stop is None:
                            return False
                        case _:
                            pass
                return True
            # while cgstree.north is not None and cgstree.east is not None and cgstree.west is not None and cgstree.south is not None and cgstree.stop is not None:
            while isCompletelyExpanded(cgstree) is True:
                children=[(cgstree.north, "North"),(cgstree.east, "East"),(cgstree.south, "South"),(cgstree.west, "West"),(cgstree.stop, "Stop")]
        ...
        def Expansion(cgstree):
            ...
            for action in legal_actions:
                match action:
                    case "North" if cgstree.north is None:
                        cgstree.north=Node([cgstree.statevalue.generateSuccessor(0,action),0,1])
                        cgstree.north.parent=cgstree
                    case "East" if cgstree.east is None:
                        cgstree.east=Node([cgstree.statevalue.generateSuccessor(0,action),0,1])
                        cgstree.east.parent=cgstree
                    case "West" if cgstree.west is None:
                        cgstree.west=Node([cgstree.statevalue.generateSuccessor(0,action),0,1])
                        cgstree.west.parent=cgstree
                    case "South" if cgstree.south is None:
                        cgstree.south=Node([cgstree.statevalue.generateSuccessor(0,action),0,1])
                        cgstree.south.parent=cgstree
                    case "Stop" if cgstree.stop is None:
                        cgstree.stop=Node([cgstree.statevalue.generateSuccessor(0,action),0,1])
